...
class SoftwareModelForm(forms.ModelForm):
    ...
    sw_portsts_fk = ModelChoiceField(label='Status', queryset=SWPortfolioStatus.objects.all(), initial=0, required=False)
    sw_portcat_fk = ModelChoiceField(label='Category', queryset=SWPortfolioCategory.objects.all(), initial=0, required=False)
