...
class SoftwareModelForm(forms.ModelForm):
    ...
    sw_vend_fk = ModelChoiceField(label='Vendor', queryset=Vendor.objects.all().order_by('swvend_name'), initial=0, required=True)
    ...
    sw_repl_vend_fk = ModelChoiceField(label='Replacement Vendor', queryset=Vendor.objects.all().order_by('swvend_name'), initial=0, required=False)
    ...
    sw_swfreq_fk = ModelChoiceField(label='Agreed Release Frequency', queryset=Vendor.objects.all(), initial=0, required=False)
    sw_vendfreq_fk = ModelChoiceField(label='Vendor Release Frequency', queryset=Vendor.objects.all(), initial=0, required=False)
